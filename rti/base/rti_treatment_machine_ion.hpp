...
public:
    ...
    beam_ds->get_values("ScanMode" , sm);                  
        machine_name += rti::trim_copy(in[0]) + ":";
    ...
    */
        ...
        const float source_to_isocenter_mm = 390.0)
    {
        ...
        ///< Modulated BEAM
        for(const auto& spot : *ion_beam.get_sequence() ){
            size_t nb_histories = (scalefactor == -1) 
                                ? 1 : this->characterize_history(spot, scalefactor);

            beamsource.append_beamlet(
                this->characterize_beamlet(spot), 
                nb_histories,
                pcoord);  
        }
